...
- ZCode（智谱/Z.ai，Electron 桌面 ADE → `zcode-cli` 是真正的 per-session agent runtime）通过 `~/.zcode/cli/config.json` 做 **state-only** hook 集成（Phase 1：SessionStart / SessionEnd / UserPromptSubmit / PreToolUse / PostToolUse / Stop / Notification），config.json 的 hook schema 与 Claude Code 一致；Clawd 只把状态映射到桌宠动画，hook stdout 恒为 `{}`，**不接管权限、不弹权限气泡**。检测进程名是 `zcode-cli`（桌面壳 `ZCode` / `zcode-host-local` 不是工作进程）；session id 命名空间是 `zcode:<raw>`。远程 SSH 部署不支持 ZCode（与 Qwen 一致，仅 local + WSL）。
